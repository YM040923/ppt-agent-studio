...
When a follow-up message clearly asks to add, append, remove, or delete a slide, the runtime mutates the cached DeckSpec with `deck.add_slide` or `deck.remove_slide`, emits a new `deck.updated` revision, refreshes `preview.ready`, and exports a new `pptx.ready` artifact. Other follow-up prompts still run through the planner path until richer edit-intent routing is added.
