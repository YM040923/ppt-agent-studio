...
`tool.completed` is a safe progress event emitted after a deterministic tool succeeds and before the related product sync event. Its payload includes `tool_name`, `status`, and a short `summary`; it does not carry raw tool outputs or secrets. The desktop app displays it in chat but keeps the turn open until `error` or a final `plan.updated` whose plan status is `completed` or `failed`.
...
During development the WinUI app starts the sidecar automatically if `127.0.0.1:8765` is not already listening. The locator first checks for bundled packaged-runtime paths at `AgentRuntime/agent/src` and `AgentRuntime/python/python.exe` beside the app, then walks up from the app base directory until it finds `agent/src`, and `PPT_AGENT_RUNTIME_ROOT` can override the repository root. The Python executable defaults to bundled Python when present and otherwise falls back to `python`; set `PPT_AGENT_PYTHON` to use a specific interpreter. Development artifacts are written to the repository `artifacts/decks` directory, while a packaged runtime writes generated decks under `%LOCALAPPDATA%/PPT Agent Studio/artifacts/decks` because the installed app directory may be read-only. Set `PPT_AGENT_ARTIFACTS_DIR` to use another directory.
...
`OpenAICompatibleChatClient` posts to `{OPENAI_BASE_URL}/chat/completions` with the configured model, bearer token, and optional extra headers. It accepts standard message content strings, text content blocks, dictionary-shaped message content, and legacy compatible `choices[].text` responses. When a provider returns multiple choices, it uses the first non-blank choice with usable text instead of failing on an empty leading choice. It also accepts an injected `httpx.AsyncClient`, so runtime tests can use `httpx.MockTransport` and avoid real network calls or secret exposure. The `LLMOutlinePlanner` uses this client plus the core system prompt to request a JSON DeckSpec outline, while `FallbackOutlinePlanner` remains the default for offline MVP runs.
