...
During development the WinUI app starts the sidecar automatically if `127.0.0.1:8765` is not already listening. The locator first checks for bundled packaged-runtime paths at `AgentRuntime/agent/src` and `AgentRuntime/python/python.exe` beside the app, then walks up from the app base directory until it finds `agent/src`, and `PPT_AGENT_RUNTIME_ROOT` can override the repository root. The Python executable defaults to bundled Python when present and otherwise falls back to `python`; set `PPT_AGENT_PYTHON` to use a specific interpreter. PPTX artifacts are written to `artifacts/decks` by default; set `PPT_AGENT_ARTIFACTS_DIR` to use another directory.
