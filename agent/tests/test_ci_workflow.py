...
def test_ci_packages_and_uploads_unsigned_msix():
    ...
    assert "Smoke bundled runtime" in workflow
    assert ".\\scripts\\test-staged-agent-runtime.ps1" in workflow
    ...
    assert "Verify unsigned MSIX contents" in workflow
    assert ".\\scripts\\test-msix-package.ps1" in workflow
