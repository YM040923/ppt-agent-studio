...
def test_agent_session_creates_slide_after_numbered_slide_follow_up(tmp_path):
    session = AgentSession(session_id="session_followup_insert", deck_id="deck_followup_insert", artifact_dir=tmp_path)

    async def first_turn():
        return [event async for event in session.submit_user_message("Make a 5 slide board AI strategy deck")]

    async def second_turn():
        return [event async for event in session.submit_user_message("Create an implementation plan slide after slide 2")]

    first_events = asyncio.run(first_turn())
    original_third_title = first_events[5].payload["deck"]["slides"][2]["title"]
    events = asyncio.run(second_turn())

    assert events[2].payload == {
        "tool_name": "deck.add_slide",
        "status": "completed",
        "summary": "Added slide: Implementation Plan.",
    }
    slides = events[3].payload["deck"]["slides"]
    assert slides[2]["title"] == "Implementation Plan"
    assert slides[3]["title"] == original_third_title
    assert events[5].payload["slide_count"] == 6
    assert events[7].payload["slide_count"] == 6


def test_agent_session_creates_slide_before_numbered_slide_follow_up(tmp_path):
    session = AgentSession(session_id="session_followup_insert_before", deck_id="deck_followup_insert_before", artifact_dir=tmp_path)

    async def first_turn():
        return [event async for event in session.submit_user_message("Make a 5 slide board AI strategy deck")]

    async def second_turn():
        return [event async for event in session.submit_user_message("Create a budget risks slide before slide 2")]
    ...
    assert events[2].payload == {
        "tool_name": "deck.add_slide",
        "status": "completed",
        "summary": "Added slide: Budget Risks.",
    }
    slides = events[3].payload["deck"]["slides"]
    assert slides[1]["title"] == "Budget Risks"
    assert slides[2]["title"] == original_second_title
    assert events[5].payload["slide_count"] == 6
    assert events[7].payload["slide_count"] == 6
...
def test_agent_session_duplicates_numbered_slide_before_numbered_target(tmp_path):
    session = AgentSession(
        session_id="session_duplicate_before",
        deck_id="deck_duplicate_before",
        artifact_dir=tmp_path,
    )
    ...
    async def second_turn():
        return [event async for event in session.submit_user_message("Duplicate slide 2 before slide 4")]
    ...
    assert events[2].payload == {
        "tool_name": "deck.add_slide",
        "status": "completed",
        "summary": "Duplicated slide 2 before slide 4.",
    }
    slides = events[3].payload["deck"]["slides"]
...
def test_agent_session_moves_numbered_slide_after_numbered_target(tmp_path):
    ...
    async def second_turn():
        return [event async for event in session.submit_user_message("Move slide 2 after slide 4")]
    ...
    assert events[2].payload == {
        ...
        "status": "completed",
        "summary": "Moved slide 2 after slide 4.",
    }
    slides = events[3].payload["deck"]["slides"]
    assert [slide["title"] for slide in slides[:4]] == [
        original_slides[0]["title"],
        original_slides[2]["title"],
        original_slides[3]["title"],
        original_slides[1]["title"],
