...
def test_chat_client_parses_direct_dict_message_content_from_compatible_provider():
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            200,
            json={
                "choices": [
                    {
                        "message": {
                            "content": {
                                "type": "output_text",
                                "content": "Direct provider content.",
                            }
                        }
                    }
                ]
    ...
    assert asyncio.run(run()) == "Direct provider content."


def test_chat_client_uses_first_choice_with_text_content_from_compatible_provider():
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            200,
            json={
                "choices": [
                    {"message": {"content": []}},
                    {"message": {"content": "Usable second choice."}},
                ]
            },
        )
