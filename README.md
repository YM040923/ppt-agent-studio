...
Lines may optionally start with `export `, so shell-style `.env.local` entries such as `export OPENAI_API_KEY=...` are accepted. Inline comments after unquoted values are ignored.
